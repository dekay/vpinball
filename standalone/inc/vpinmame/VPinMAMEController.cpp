   ...
   if (index == displayCount - 1) {
      for (int i = 0; i < m_displays.size(); i++) {
         if ((m_displays[i].type & DMD) == DMD) {
            ...
            m_pDMDSurface = SDL_CreateRGBSurfaceWithFormat(0, m_dmdWidth, m_dmdHeight, 24, SDL_PIXELFORMAT_RGB24);
...
void VPinMAMEController::Render(SDL_Renderer* pRenderer)
{
   if (!m_pDMDSurface)
      return;

   SDL_LockSurface(m_pDMDSurface);

   UINT8* pixels = (UINT8*)m_pDMDSurface->pixels;
   for (int y = 0; y < m_pDMDSurface->h; ++y) {
      for (int x = 0; x < m_pDMDSurface->w; ++x) {
         int index = m_pDmdData[y * m_pDMDSurface->w + x];
         UINT8 r = m_dmdPalette[index * 3];
         UINT8 g = m_dmdPalette[index * 3 + 1];
         UINT8 b = m_dmdPalette[index * 3 + 2];

         pixels[y * m_pDMDSurface->pitch + x * 3] = r;
         pixels[y * m_pDMDSurface->pitch + x * 3 + 1] = g;
         pixels[y * m_pDMDSurface->pitch + x * 3 + 2] = b;
      }
   }

   SDL_UnlockSurface(m_pDMDSurface);

   SDL_Texture* pTexture = SDL_CreateTextureFromSurface(pRenderer, m_pDMDSurface);
...
STDMETHODIMP VPinMAMEController::Run(/*[in]*/ LONG_PTR hParentWnd, /*[in,defaultvalue(100)]*/ int nMinVersion)
{
   if (m_pPinmameGame) {
      ...
      if (status == PINMAME_STATUS::OK) {
         ...
         g_pplayer->SetDMDRenderCallback(std::bind(&VPinMAMEController::Render, this, std::placeholders::_1));
