...
void GraphicsPath::AddClosedCurve(const std::vector<SDL_FPoint>& points)
{
   AddClosedCurve(points, 0.5f);
}

void GraphicsPath::AddClosedCurve(const std::vector<SDL_FPoint>& points, float tension)
{
   if (points.size() < 3)
      return;
...
void GraphicsPath::AddPolygon(const std::vector<SDL_FPoint>& points)
{
   if (points.size() < 3)
      return;
...
int GraphicsPath::FlattenPath(GraphicsPath* pPath, B2S::Matrix* pMatrix, float flatness)
{
   ...
   std::vector<PathPointType> types;
