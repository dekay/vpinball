...
void Player::CreateWnd(HWND parent /* = 0 */)
{
...
#ifdef __STANDALONE__
   flags = SDL_WINDOW_SKIP_TASKBAR | SDL_WINDOW_BORDERLESS | SDL_WINDOW_HIDDEN;
   ...
   if (b2s) {
      m_pB2SWindow = SDL_CreateWindow("vpx_b2s",
         ...
         LoadValueWithDefault(regKey[RegName::Standalone], "B2SHeight"s, 0), flags);
...
#ifdef __STANDALONE__
void Player::SetB2SRenderCallback(B2SRenderCallback callback)
{
   if (!m_pB2SWindow || !m_pB2SRenderer)
      return;

   m_b2sRenderCallback = callback;
   if (callback)
      SDL_ShowWindow(m_pB2SWindow);
   else
      SDL_HideWindow(m_pB2SWindow);

   SDL_RaiseWindow(m_sdl_playfieldHwnd);
}

void Player::SetDMDRenderCallback(DMDRenderCallback callback)
{
